...
export class ElasticSearch {
  ...
  public async checkConnection(): Promise<void> {
    ...
    while (!isConnected) {
      try {
        this.log.info('NotificationService Connecting to ElasticSearch...');
        const health: ClusterHealthResponse = await this.elasticSearchClient.cluster.health({});
        this.log.info(`NotificationService ElasticSearch health status - ${health.status}`);
        isConnected = true;
      } catch (error) {
        this.log.error('Connection to ElasticSearch failed, retrying in 3 seconds...');
        this.log.log('error', 'NotificationService checkConnection method error: ', error);
